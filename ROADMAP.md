...
| select2 se puede compilar en el bundle del tema | Encolarlo como archivo suelto no necesita configuración, pero con un empaquetador es una excepción que no pasa por el build. Con `forja/enqueue_select2` a false y el import de `apros-forja/js/vendor/select2` entra por el bundle. **jQuery se marca como externo**: lo pone WordPress y duplicarlo rompería los plugins que dependen de esa instancia. |
